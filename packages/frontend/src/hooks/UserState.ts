...
export default class UserState {
    ...
    public genUserStateTransitionProof = async (
        ...
        const leafHash = poseidon2([
            (stateTree as any).root,
            (epochTree as any).root,
        ])
        ...
        if (leaf) {
            historyTreeProof = await (historyTree as any).createProof(
                leaf.index
            )
        } else {
            ...
            await (historyTree as any).insert(leafHash)
            historyTreeProof = await (historyTree as any).createProof(leafCount)
        }
            ...
            epochKeys.map(async (epochKey, i) => {
                ...
                const proof =
                    epochKeyLeafIndices[i] !== -1
                        ? (epochTree as any).createProof(epochKeyLeafIndices[i])
                        : {
                              pathIndices: Array(
        ...
        const stateTreeProof = (stateTree as any).createProof(latestLeafIndex)
        const circuitInputs = {
            ...
            epoch_tree_root: (epochTree as any).root,
    ...
    public genProveReputationProof = async (options: {
        ...
        const stateTreeProof = (stateTree as any).createProof(leafIndex)
    ...
    public genEpochKeyProof = async (
        ...
        const leafIndex = await this.latestStateTreeLeafIndex(epoch, attesterId)
        const data = await this.getData(epoch - 1, attesterId)
        const proof = (tree as any).createProof(leafIndex)
        const circuitInputs = {
            identity_secret: this.id.secret,
            data,
